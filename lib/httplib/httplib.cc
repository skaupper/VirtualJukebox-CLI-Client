...
namespace httplib {
    ...
    namespace detail {
        ...
        bool keep_alive(socket_t sock) {
            ...
            while (true) {
                ...
                if (val < 0) {
                    ...
                    auto timeout =
                        CPPHTTPLIB_KEEPALIVE_TIMEOUT_SECOND * 1000 + CPPHTTPLIB_KEEPALIVE_TIMEOUT_USECOND / 1000;
        ...
        template<typename T>
        bool process_server_socket_core(socket_t sock, size_t keep_alive_max_count, T callback) {
        ...
        template<typename T>
        bool process_server_socket(socket_t sock, size_t keep_alive_max_count, time_t read_timeout_sec,
                                          time_t read_timeout_usec, time_t write_timeout_sec, time_t write_timeout_usec,
                                          T callback) {
            return process_server_socket_core(
                sock, keep_alive_max_count, [&](bool close_connection, bool connection_closed) {
        ...
        socket_t create_socket(const char *host, int port, int socket_flags, bool tcp_nodelay,
        ...
        socket_t create_client_socket(const char *host, int port, bool tcp_nodelay, SocketOptions socket_options,
                                             time_t timeout_sec, time_t timeout_usec, const std::string &intf) {
            return create_socket(
                host, port, 0, tcp_nodelay, socket_options, [&](socket_t sock, struct addrinfo &ai) -> bool {
                    if (!intf.empty()) {
#ifndef _WIN32
                        auto ip = if2ip(intf);
                        if (ip.empty()) {
        ...
        void get_remote_ip_and_port(const struct sockaddr_storage &addr, socklen_t addr_len, std::string &ip,
                                           int &port) {
        ...
        bool has_header(const Headers &headers, const char *key) { return headers.find(key) != headers.end(); }

        const char *get_header_value(const Headers &headers, const char *key, size_t id = 0,
        ...
        bool read_content(Stream &strm, T &x, size_t payload_max_length, int &status, Progress progress,
                          ContentReceiver receiver, bool decompress) {
        ...
        template<typename T>
        ssize_t write_content_chunked(Stream &strm, ContentProvider content_provider, T is_shutting_down) {
            size_t offset                = 0;
        ...
        std::pair<size_t, size_t> get_range_offset_and_length(const Request &req, size_t content_length,
                                                                     size_t index) {
        ...
        std::string make_multipart_ranges_data(const Request &req, Response &res, const std::string &boundary,
        ...
        size_t get_multipart_ranges_data_length(const Request &req, Response &res, const std::string &boundary,
                                                       const std::string &content_type) {
        ...
        template<typename T>
        bool write_multipart_ranges_data(Stream &strm, const Request &req, Response &res,
        ...
        std::pair<size_t, size_t> get_range_offset_and_length(const Request &req, const Response &res,
                                                                     size_t index) {
            auto r = req.ranges[index];
        ...
        std::string message_digest(const std::string &s, Init init, Update update, Final final,
    ...
    bool Request::has_header(const char *key) const { return detail::has_header(headers, key); }
    ...
    bool Request::has_param(const char *key) const { return params.find(key) != params.end(); }
    ...
    bool Response::has_header(const char *key) const { return headers.find(key) != headers.end(); }
    ...
    void Response::set_content_provider(size_t in_length, ContentProvider provider,
    ...
    void Response::set_chunked_content_provider(ChunkedContentProvider provider,
                                                       std::function<void()> resource_releaser) {
        content_length_   = 0;
    ...
    void Server::set_file_request_handler(Handler handler) { file_request_handler_ = std::move(handler); }

    void Server::set_error_handler(Handler handler) { error_handler_ = std::move(handler); }

    void Server::set_tcp_nodelay(bool on) { tcp_nodelay_ = on; }

    void Server::set_socket_options(SocketOptions socket_options) { socket_options_ = socket_options; }
    ...
    void Server::set_keep_alive_max_count(size_t count) { keep_alive_max_count_ = count; }
    ...
    void Server::set_payload_max_length(size_t length) { payload_max_length_ = length; }
    ...
    bool Server::write_content_with_provider(Stream &strm, const Request &req, Response &res,
                                                    const std::string &boundary, const std::string &content_type) {
    ...
    bool Server::read_content_with_content_receiver(Stream &strm, Request &req, Response &res,
    ...
    bool Server::handle_file_request(Request &req, Response &res, bool head) {
    ...
    socket_t Server::create_server_socket(const char *host, int port, int socket_flags,
                                                 SocketOptions socket_options) const {
        return detail::create_socket(host, port, socket_flags, tcp_nodelay_, socket_options,
    ...
    bool Server::dispatch_request_for_content_reader(Request &req, Response &res, ContentReader content_reader,
    ...
    bool Server::process_request(Stream &strm, bool close_connection, bool &connection_closed,
    ...
    bool Server::process_and_close_socket(socket_t sock) {
        auto ret = detail::process_server_socket(
            sock, keep_alive_max_count_, read_timeout_sec_, read_timeout_usec_, write_timeout_sec_, write_timeout_usec_,
    ...
    Client::Client(const std::string &host, int port, const std::string &client_cert_path,
                          const std::string &client_key_path)
        : host_(host), port_(port), host_and_port_(host_ + ":" + std::to_string(port_)),
    ...
    socket_t Client::create_client_socket() const {
        if (!proxy_host_.empty()) {
            return detail::create_client_socket(proxy_host_.c_str(), proxy_port_, tcp_nodelay_, socket_options_,
    ...
    bool Client::send(const Request &req, Response &res) {
        ...
        auto ret =
            process_socket(socket_, [&](Stream &strm) { return handle_request(strm, req, res, close_connection); });
    ...
    std::shared_ptr<Response> Client::send_with_content_provider(const char *method, const char *path,
    ...
    bool Client::process_request(Stream &strm, const Request &req, Response &res, bool close_connection) {
        ...
        if (req.method != "HEAD" && req.method != "CONNECT") {
            auto out = req.content_receiver ? static_cast<ContentReceiver>(
                           [&](const char *buf, size_t n) { return req.content_receiver(buf, n); })
    ...
    std::shared_ptr<Response> Client::Head(const char *path) { return Head(path, Headers()); }
    ...
    std::shared_ptr<Response> Client::Post(const char *path) { return Post(path, std::string(), nullptr); }

    std::shared_ptr<Response> Client::Post(const char *path, const std::string &body, const char *content_type) {
        return Post(path, Headers(), body, content_type);
    }

    std::shared_ptr<Response> Client::Post(const char *path, const Headers &headers, const std::string &body,
                                                  const char *content_type) {
    ...
    std::shared_ptr<Response> Client::Post(const char *path, size_t content_length,
                                                  ContentProvider content_provider, const char *content_type) {
        return Post(path, Headers(), content_length, content_provider, content_type);
    }

    std::shared_ptr<Response> Client::Post(const char *path, const Headers &headers, size_t content_length,
                                                  ContentProvider content_provider, const char *content_type) {
    ...
    std::shared_ptr<Response> Client::Post(const char *path, const Headers &headers,
    ...
    std::shared_ptr<Response> Client::Put(const char *path) { return Put(path, std::string(), nullptr); }

    std::shared_ptr<Response> Client::Put(const char *path, const std::string &body, const char *content_type) {
        return Put(path, Headers(), body, content_type);
    }

    std::shared_ptr<Response> Client::Put(const char *path, const Headers &headers, const std::string &body,
                                                 const char *content_type) {
        return send_with_content_provider("PUT", path, headers, body, 0, nullptr, content_type);
    }

    std::shared_ptr<Response> Client::Put(const char *path, size_t content_length,
                                                 ContentProvider content_provider, const char *content_type) {
        return Put(path, Headers(), content_length, content_provider, content_type);
    }

    std::shared_ptr<Response> Client::Put(const char *path, const Headers &headers, size_t content_length,
                                                 ContentProvider content_provider, const char *content_type) {
    ...
    std::shared_ptr<Response> Client::Patch(const char *path, const Headers &headers, const std::string &body,
                                                   const char *content_type) {
        return send_with_content_provider("PATCH", path, headers, body, 0, nullptr, content_type);
    }

    std::shared_ptr<Response> Client::Patch(const char *path, size_t content_length,
                                                   ContentProvider content_provider, const char *content_type) {
        return Patch(path, Headers(), content_length, content_provider, content_type);
    }

    std::shared_ptr<Response> Client::Patch(const char *path, const Headers &headers, size_t content_length,
                                                   ContentProvider content_provider, const char *content_type) {
    ...
    std::shared_ptr<Response> Client::Delete(const char *path, const std::string &body,
                                                    const char *content_type) {
    ...
    std::shared_ptr<Response> Client::Delete(const char *path, const Headers &headers, const std::string &body,
                                                    const char *content_type) {
        Request req;
        req.method  = "DELETE";
        req.headers = headers;
    ...
    std::shared_ptr<Response> Client::Options(const char *path) { return Options(path, Headers()); }
    ...
    void Client::set_timeout_sec(time_t timeout_sec) { set_connection_timeout(timeout_sec, 0); }
    ...
    void Client::set_socket_options(SocketOptions socket_options) { socket_options_ = socket_options; }
 ...
 */
    ...
    namespace detail {
        ...
        void ssl_delete(std::mutex &ctx_mutex, SSL *ssl, bool process_socket_ret) {
        ...
        template<typename T>
        bool process_server_socket_ssl(SSL *ssl, socket_t sock, size_t keep_alive_max_count,
    ...
    bool SSLServer::process_and_close_socket(socket_t sock) {
        ...
        if (ssl) {
            auto ret = detail::process_server_socket_ssl(
                ssl, sock, keep_alive_max_count_, read_timeout_sec_, read_timeout_usec_, write_timeout_sec_,
    ...
    SSLClient::SSLClient(const std::string &host, int port, const std::string &client_cert_path,
                                const std::string &client_key_path)
    ...
    void SSLClient::set_ca_cert_path(const char *ca_cert_file_path, const char *ca_cert_dir_path) {
    ...
    bool SSLClient::process_socket(Socket &socket, std::function<bool(Stream &strm)> callback) {
        assert(socket.ssl);
