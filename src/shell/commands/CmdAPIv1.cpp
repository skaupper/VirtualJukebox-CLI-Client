...
CmdAPIv1::CmdAPIv1(std::istream &in, const std::string &prompt)
    : ShellCommand(), mPrompt(prompt), mShell(mPrompt), mInput(in) {
    api::v1::Initialize();
...
ShellCommandDetails CmdAPIv1::getCommandDetails() const {
    ...
    details.description = "Connect to a running VirtualJukebox server "
                          "with the given credentials and start a subshell afterwards.";
    details.usage = getTrigger() + " <server_ip> <server_port> <nickname> [<admin_password>]";
    details.parameterDescription["<server_ip>"] = "The IP of the VirtualJukebox server. May "
                                                  "also be a valid hostname.";
    details.parameterDescription["<server_port>"] = "The port of the VirtualJukebox server. If the "
                                                    "value is not an integer, an exception is thrown.";
    details.parameterDescription["<nickname>"] = "A non-unique nickname which is used "
                                                 "when connecting to the server.";
    details.parameterDescription["<admin_password>"] = "If the admin password is given, "
                                                       "try to connect as an administrator "
                                                       "(and fail if the password is wrong).";
    return details;
}
